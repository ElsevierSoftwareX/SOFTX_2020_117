...
# accessed 27/11/2015
#      - additions by Anna at the bottom.
###############################################################
import numpy as np
...
################### Added by Anna #########################
def Peakvals(x,y):
    _max, _min = peakdetect(y, x, 1)#clean data - small lookahead and delta
    peakx = [p[0] for p in _max]
    peaky = [p[1] for p in _max]
    print('\npeaks at the following frequencies:' + str(peakx))
    legend.append('" peaks')
    pl.plot(peakx,peaky,'o', color=lcol)